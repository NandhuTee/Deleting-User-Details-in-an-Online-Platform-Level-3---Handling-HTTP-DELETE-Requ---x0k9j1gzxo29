...
app.delete("/api/v1/details/:id", (req, res) => {
  ...
  const userToDelete = userDetails.find(
    (entry) => entry.id === id
  );
  if(!userToDelete){
  ...
  const updatedUsers = userDetails.filter((entry) => {
    entry.id !== id
    
  })
  fs.writeFile(`${__dirname}/data/userDetails.json`, JSON.stringify(updatedUsers), (err) => {
